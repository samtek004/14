...
** 01/18/2023	BRINH1		Fixed a few performance issues
******************************************************************************/ 
BEGIN  
    SET NOCOUNT ON  
    ...
    INSERT INTO @om_serv_type_tbl  
        (om_serv_type_cd)  
    SELECT value  
    FROM code.dbo.fn_split(  
            (SELECT value  
            FROM [reference].[dbo].[app_config_settings] acs  
            WHERE acs.[app_name]='PowerPage'  
            AND [key]='ExcludeServiceTypeCodes'  
            ),',')  
  
    CREATE TABLE #tmp_service_summary  
        (customerserviceid BIGINT  
        ,customerid VARCHAR(10)  
        ,serviceorderid VARCHAR(10)  
        ,serviceid INT  
    ...
    CREATE TABLE #tmp_omcustomerservicemaster  
        ([CustomerServiceID] [bigint] NOT NULL,  
        [CustomerID] [varchar](10) NOT NULL,  
        [ServiceID] [int] NOT NULL,  
        [WasteProfileID] [varchar](15) NULL,  
        [Term] [int] NULL,  
        [NextScheduledYear] [int] NULL,  
        [NextScheduledWeek] [int] NULL,  
        [LastServiceDate] [datetime] NULL,  
        [AssociatedBranchID] [varchar](10) NOT NULL,  
        [Department] [varchar](50) NULL,  
        [PlacementDate] [datetime] NULL,  
        [IsOnServicePlan] [bit] NOT NULL,  
        [UsageFrequency] [int] NULL,  
        [IsVendorService] [bit] NULL,  
        [MachineID] [int] NULL)  
    ...
    CREATE TABLE #temp_omserviceordermaster(  
        CustomerID VARCHAR(10),  
 ...
 CREATE TABLE #temp_omserviceorderdetail  
 (  
 temp_omserviceorderdetail INT PRIMARY KEY IDENTITY (1,1) NOT NULL,  
 serviceid INT,  
 ...
 serviceorderid INT  
    ...
    SELECT @target_cd =  
        (SELECT TOP(1) COALESCE(target_cd,'')  
        FROM reference.dbo.dupe_account_targets dat  
        WHERE dat.dupe_cd = @CustomerId  
        AND dat.dupe_xprtn_dt > @dt  
        ORDER BY dupe_account_target_id DESC)  
  
    SELECT @source_cd =  
        (SELECT TOP(1) COALESCE(dupe_cd,'')  
        FROM reference.dbo.dupe_account_targets dat  
        WHERE dat.target_cd = @CustomerId  
        AND dat.dupe_xprtn_dt > @dt  
        ORDER BY dupe_account_target_id DESC)  
    ...
    INSERT INTO #tmp_omcustomerservicemaster  
        (CustomerServiceID  
        ,customerid  
        ,serviceid  
        ,WasteProfileID  
        ...
        ,IsVendorService  
        ,MachineID)  
    SELECT omcsm.CustomerServiceID  
            ,omcsm.customerid  
            ,omcsm.serviceid  
    ...
    AND omcsm.servicestatus='ACTIVE'  
    AND omcsm.InactivationDate IS NULL  
  
  
  
 INSERT INTO #temp_omserviceorderdetail  
 (serviceid, customerserviceid, serviceorderid)  
 SELECT omsod.serviceid, omsod.customerserviceid, omsod.serviceorderid  
 FROM  #tmp_omcustomerservicemaster omscm  
 INNER JOIN skomdb.dbo.OmserviceOrderDetails omsod  
 ON omscm.customerserviceid = omsod.customerserviceid  
 AND omscm.serviceid = omsod.serviceid  
 AND omsod.IsActive = 1  
  
    INSERT INTO #temp_omserviceordermaster(  
        CustomerID    
        ,ServiceOrderID  
        ,ScheduledWeek        
        ,ScheduledYear  
    ...
    SELECT   
        ...
        ,omsom.CreateUser  
    FROM [skomdb].[dbo].[omserviceordermaster] omsom    
    WHERE omsom.CustomerID IN (@CustomerId, @target_cd, @source_cd)        
        AND omsom.BranchID = @branchid   
        AND ((omsom.ScheduledYear >= @yearnow AND omsom.ScheduledWeek >= @weeknow) OR  (omsom.ScheduledYear > @yearnow ))   
        AND ISNULL(omsom.VoidReasonId, 0) = 0     
        AND ISNULL(omsom.OMServiceOrderTypeLovID,-1) <> 2  
  
  
  
  
    CREATE CLUSTERED INDEX #tmp_omcustomerservicemaster_ind1  
    ON #tmp_omcustomerservicemaster  
        (serviceid ASC,  
    ...
    SELECT DISTINCT  
        ...
        ,omcsm.customerid  
        ,CAST(omsom.serviceorderid AS VARCHAR(10))  
        ,omcsm.serviceid  
        ,omcsm.WasteProfileID  
        ...
        ,omcsm.nextscheduledyear  
        ,CASE  
            WHEN omsom.RecordStatus = 'COMPLETE' THEN DATEPART(ISO_WEEK,omcsm.LastServiceDate)  
            ELSE  DATEPART(ISO_WEEK,omcsm.LastServiceDate)  
        END  AS lastserviceweek  
        ...
        ,ca.zip  
        ,CASE  
            WHEN omsom.ordersource = 'ServiceRequest' THEN 1  
            ELSE 0  
        END AS is_adhoc_indcr  
        ...
        ,omcsm.IsOnServicePlan  
        ,CASE  
            WHEN omcsm.UsageFrequency = '52' THEN 'Once Per Year'  
        ...
        ,CASE  
            WHEN (pm.part_name LIKE '%DIY%' or pm.part_name LIKE '%DO-IT-YOURSELF%') and pm.actv_indcr = 1 THEN 1  
            ELSE 0  
        END as IsDIY  
        ,omm.MachineSerialNumber AS SerialNumber  
        ,cl.custmr_co_cd  
        ,omcsm.IsVendorService  
    FROM #tmp_omcustomerservicemaster omcsm  
    LEFT JOIN #temp_omserviceorderdetail omsod  
        ON omcsm.serviceid = omsod.serviceid  
        AND omcsm.customerserviceid = omsod.customerserviceid  
   LEFT JOIN #temp_omserviceordermaster omsom  
        ON omsod.serviceorderid = omsom.serviceorderid  
    INNER JOIN order_mgmt.pricing.part_master pm  
        ON pm.part_no = omcsm.ServiceID  
    INNER JOIN order_mgmt.pricing.part_hierarchy ph  
        ON ph.part_hierarchy_id = pm.part_hierarchy_id  
    INNER JOIN reference.dbo.company_addr ca  
        ON omcsm.customerid=ca.co_cd  
        AND ca.addr_type='CORP'  
    INNER JOIN reference.dbo.company_link cl  
        ON omcsm.customerid = cl.genrtr_co_cd   
    LEFT JOIN reference.dbo.emplye_master em  
        ON omsom.CreateUser = em.usrnam  
    LEFT JOIN reference.dbo.emplye_email ee  
        ON em.emplye_id= ee.emplye_id  
        AND ee.email_address_type = 'BUSN'  
        AND ee.actv_indcr = 1  
    ...
    OUTER APPLY  
        ...
        INNER JOIN skomdb.dbo.OMMachine m  
            ON aa.MachineID = m.machineid  
            AND aa.actv_indcr = 1  
        INNER JOIN order_mgmt.pricing.part_master p  
            ON p.part_no = aa.serviceID  
        WHERE aa.customerserviceid=omcsm.customerserviceid  
        AND aa.PullDate IS NULL  
    ...
    OUTER APPLY  
        ...
        INNER JOIN skomdb.dbo.OMMachine m  
            ON aa.MachineID = m.machineid  
            AND aa.actv_indcr = 1  
            AND m.MachineAssetTag IS NOT NULL  
        WHERE aa.customerserviceid=omcsm.customerserviceid  
        AND aa.PullDate IS NULL  
        ORDER BY m.MachineAssetTag  
        ) mat  
   
  
 UPDATE tss  
 SET tss.profile_desc = wp.prflMtrlName  
 ,tss.profile_status  = CASE  
    WHEN COALESCE(pv.[status],wp.prflStaCd) IN ('E') OR COALESCE(pv.expiration_date,@dt) < @dt OR COALESCE(asps.expired,'')='E' THEN 'E'  
    ELSE 'A'  
    END   
 FROM #tmp_service_summary tss  
 INNER JOIN [cwt].[dbo].[profile] wp  
    ON tss.profile_no = wp.prflNum  
 LEFT JOIN choice.dbo.profil_vendor pv    
 ON tss.profile_no = pv.vendor_profil_no     
 AND tss.CustomerID = pv.genrtr      
 AND pv.vendor_id='CH'    
  OUTER APPLY     
   (SELECT TOP 1     
   CASE     
    WHEN (COALESCE(p.[status],'') IN ('E') OR COALESCE(p.expiration_date,@dt) < @dt ) THEN 'E'     
    ELSE 'A'     
   END as expired      
   ...
   INNER JOIN choice.dbo.profil_vendor p       
    ...
    AND P.genrtr = tss.customerid     
    AND p.vendor_id = 'CH'      
   WHERE sa.customerserviceid=tss.customerserviceid       
   AND sa.actv_indcr = 1       
   ORDER BY     
    CASE     
  WHEN (COALESCE(p.[status],'') IN ('E') OR COALESCE(p.expiration_date,@dt) < @dt ) THEN 2     
  ELSE 1     
    END      
   -- If any non-expired associated profile exists, prefer that one. Otherwise, if any expired profile exists, return expired status.      
    ...
    CREATE INDEX ##tmp_service_summary_ind1  
    ON #tmp_service_summary  
        (serviceid ASC)  
  
    CREATE INDEX ##tmp_service_summary_ind2  
    ON #tmp_service_summary  
        (ServiceOrderID ASC)  
    ...
    LEFT JOIN reference.dbo.emplye_master em  
        ON osm.CreateUser = em.usrnam  
    LEFT JOIN reference.dbo.emplye_email ee  
        ON ee.emplye_id = em.emplye_id  
        AND ee.email_address_type = 'BUSN'  
        AND ee.actv_indcr =1  
    LEFT JOIN [skomdb].[dbo].[OMServiceOrderDetails] osd  
        ON osm.ServiceOrderID = osd.ServiceOrderID  
    WHERE customerid IN (@customerId, @target_Cd, @source_cd)  
    AND osm.RecordStatus <> 'COMPLETE'  
    AND osm.ActualScheduleDate > = @weekSundayStDate  
    UNION All  
    SELECT osm.ActualScheduleDate  
        ,osm.ServiceOrderID  
        ,osd.ServiceID  
    ...
    SELECT DISTINCT tss.customerid  
        ...
        ,BD.Dept  
        ,CASE  
            WHEN tss.recordstatus = 'COMPLETE' THEN DATEPART(ISO_WEEK,COALESCE(tss.completed_dt,ct.completed_dt))  
            ELSE  tss.scheduledweek  
        END  AS scheduledweek  
        ,CASE  
            WHEN tss.recordstatus = 'COMPLETE' THEN DATEPART(YEAR,COALESCE(tss.completed_dt,ct.completed_dt))  
            ELSE  tss.ScheduledYear  
        END  AS ScheduledYear  
        ,tss.NextScheduledWeek  
        ,tss.nextscheduledyear  
        ,CASE  
            WHEN tss.recordstatus = 'COMPLETE' THEN COALESCE(tss.lastserviceweek,DATEPART(ISO_WEEK,ct.ActualScheduleDate))  
            ELSE  COALESCE(tss.lastserviceweek,DATEPART(WK,ct.ActualScheduleDate))  
        ...
        ,CASE  
            WHEN pml.part_material_group_id IS NOT NULL THEN va.remarks  
            ELSE NULL  
        END AS route_name  
        ,CASE  
            WHEN pml.part_material_group_id IS NOT NULL THEN oa.truck_stop  
            ELSE NULL  
        END  as stop_number  
        ...
        ,CASE  
            WHEN tss.hierarchy_value LIKE '1000%' OR tss.hierarchy_value LIKE '2000%' OR tss.hierarchy_value LIKE '7000%' OR tss.hierarchy_value LIKE '4000%'  
                    OR tss.hierarchy_value LIKE '5000%' OR tss.hierarchy_value LIKE '5100%' OR tss.hierarchy_value LIKE '6100%' THEN 1  
            ELSE 0  
        END AS is_part_washer  
        ...
        ,tss.IsVendorService  
    FROM #tmp_service_summary tss  
    LEFT JOIN #tmp_summary_detail ct  
        ON ct.ServiceID  = tss.ServiceID  
        AND ct.RowORDER = 1  
        AND ct.RecordStatus = 'COMPLETE'  
    LEFT JOIN order_mgmt.dbo.order_alignment oa  
        ON tss.ServiceOrderID = oa.order_no  
        AND oa.actv_indcr =1  
    LEFT JOIN order_mgmt.dbo.vhcle_alignment va  
        ON oa.vhcle_alignment_id = va.vhcle_alignment_id  
        AND va.actv_indcr =1  
    INNER JOIN order_mgmt.pricing.part_master pm  
        ON tss.serviceid=pm.part_no  
        AND pm.actv_indcr=1  
    OUTER APPLY  
        (SELECT TOP(1) part_material_group_id  
        FROM order_mgmt.pricing.part_material_group_link pl  
        WHERE pm.part_master_id = pl.part_master_id  
        AND pl.part_material_group_id IN (74,75,76,83)  
        ORDER BY part_material_group_id  
        ) pml  
    ...
    OUTER APPLY  
        (SELECT TOP(1) ActualScheduleDate  
        FROM #tmp_summary_detail  
        WHERE ServiceID  = tss.ServiceID  
        AND RecordStatus <> 'COMPLETE'  
        AND ActualScheduleDate > = DATEADD(WEEK, DATEDIFF(DAY,0, GETDATE())/7,0)-1  
        ORDER BY ServiceID  
        ) CurrSvcDt  
    WHERE NOT EXISTS  
        (SELECT TOP(1) 1  
        FROM  @om_serv_type_tbl ostt  
        WHERE ostt.om_serv_type_cd=tss.om_serv_type_cd  
        ORDER BY ostt.om_serv_type_cd)  
    ORDER BY  
        tss.serviceorderid  
        ,tss.serviceid  
        ,scheduledweek  
    ...
    DROP TABLE #tmp_omcustomerservicemaster  
    DROP TABLE #temp_omserviceordermaster  
  
    RETURN @err -- Zero unless an error occurred in final select statement  
END  
GO
